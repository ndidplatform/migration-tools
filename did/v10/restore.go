...
import (
	...
	protoParam "github.com/ndidplatform/migration-tools/did/v10/protos/param"
	protoTm "github.com/ndidplatform/migration-tools/did/v10/protos/tendermint"
	"github.com/ndidplatform/migration-tools/did/v10/tm_client"
	"github.com/ndidplatform/migration-tools/did/v10/types"
...
func Restore(
	...
	data, err := io.ReadAll(ndidKeyFile)
	if err != nil {
		return err
	}
	...
	dataMaster, err := io.ReadAll(ndidMasterKeyFile)
	if err != nil {
		return err
	}
...
func SetNodeKeys(
	ndidID string,
	nodeSigningMasterPublicKeyFilepath string,
	nodeSigningMasterAlgorithm string,
	nodeSigningPublicKeyFilepath string,
	nodeSigningAlgorithm string,
	nodeEncryptionPublicKeyFilepath string,
	nodeEncryptionAlgorithm string,
	keyDir string,
	tendermintRPCHost string,
	tendermintRPCPort string,
) (err error) {
	...
	// new public keys
	ndidSigningPublicKeyFile, err := os.Open(nodeSigningPublicKeyFilepath)
	if err != nil {
		return err
	}
	defer ndidSigningPublicKeyFile.Close()
	ndidSigningNodePublicKey, err := io.ReadAll(ndidSigningPublicKeyFile)
	if err != nil {
		return err
	}
	ndidSigningMasterPublicKeyFile, err := os.Open(nodeSigningMasterPublicKeyFilepath)
	if err != nil {
		return err
	}
	defer ndidSigningMasterPublicKeyFile.Close()
	ndidSigningNodeMasterPublicKey, err := io.ReadAll(ndidSigningMasterPublicKeyFile)
	if err != nil {
		return err
	}
	ndidEncryptionPublicKeyFile, err := os.Open(nodeEncryptionPublicKeyFilepath)
	if err != nil {
		return err
	}
	defer ndidEncryptionPublicKeyFile.Close()
	ndidEncryptionNodePublicKey, err := io.ReadAll(ndidEncryptionPublicKeyFile)
	if err != nil {
		return err
	}

	// TODO: validate key algorithm - only RSA is supported (for now)
	...
	err = updateNode(
		tmClient,
		ndidMasterPrivKey,
		string(ndidSigningNodePublicKey),
		string(types.SignatureAlgorithmRSAPKCS1V15SHA256),
		string(ndidSigningNodeMasterPublicKey),
		string(types.SignatureAlgorithmRSAPKCS1V15SHA256),
		string(ndidEncryptionNodePublicKey),
		"RSAES_PKCS1_V1_5",
		ndidID,
	)
	if err != nil {
...
func initNDID(
	...
	chainHistoryData, err := os.ReadFile(path.Join(backupDataDir, chainHistoryFileName))
	if err != nil {
		return err
	}
	...
	initNDIDparam.SigningPublicKey = string(ndidPublicKeyBytes)
	initNDIDparam.SigningAlgorithm = string(types.SignatureAlgorithmRSAPKCS1V15SHA256)
	initNDIDparam.SigningMasterPublicKey = string(ndidMasterPublicKeyBytes)
	initNDIDparam.SigningMasterAlgorithm = string(types.SignatureAlgorithmRSAPKCS1V15SHA256)
	initNDIDparam.EncryptionPublicKey = string(ndidPublicKeyBytes)
	initNDIDparam.EncryptionAlgorithm = "RSAES_PKCS1_V1_5"
...
func updateNode(
	...
	nodeSigningAlgorithm string,
	nodeSigningMasterPublicKeyPem string,
	nodeSigningMasterAlgorithm string,
	...
	updateNodeParam.SigningPublicKey = nodeSigningPublicKeyPem
	updateNodeParam.SigningAlgorithm = nodeSigningAlgorithm
	updateNodeParam.SigningMasterPublicKey = nodeSigningMasterPublicKeyPem
	updateNodeParam.SigningMasterAlgorithm = nodeSigningMasterAlgorithm
	updateNodeParam.EncryptionPublicKey = nodeEncryptionPublicKeyPem
	updateNodeParam.EncryptionAlgorithm = nodeEncryptionAlgorithm
	...
	pssh.Write(PSSmessage)
	hashed := pssh.Sum(nil)
	signature, err := rsa.SignPKCS1v15(rand.Reader, nodeKey, newhash, hashed)
	if err != nil {
		return err
	}
