...
import (
	...
	"github.com/ndidplatform/migration-tools/backup_v3_for_v4/utils"
)
...
func readStateDBAndWriteToFile(curChain chainHistoryDetail) {
	...
	for ; itr.Valid(); itr.Next() {
		...
		case strings.Contains(string(key), "Request") && strings.Contains(string(key), "versions"):
			...
			err := protosV4.Unmarshal([]byte(value), &keyVersions)
			if err != nil {
				panic(err)
			}
			...
			newReqVersionsValue, err := protosV4.DeterministicMarshal(&keyVersions)
			if err != nil {
				panic(err)
			}
